...
#### Fixed
- **Correlation scanner missed +6.86% BTC rally** — only checked current 4h candle
  - Now checks 1h/4h/12h/24h rolling windows with scaled thresholds
  - Added ETH as second leader with ecosystem-specific alt lists
  - Cron tightened from 10min to 3min for time-sensitive lag trades
- **xyz: assets could cascade-crash all scanners** — 5h outage on Mar 4
  - Prescreener now isolates xyz: into separate group_xyz
  - All scanners hard-filter xyz: from loaded group names
  - Per-asset try/except in all scanner loops prevents cascade failures
- **Scanner resilience** — one failing asset no longer kills entire scan run

#### Key Lessons
- Single-candle price checks miss sustained multi-candle moves
- BTC-only correlation misses ETH-led ecosystem moves
- 10min scan interval too slow for correlation lag trades (close in 2-5 min)
- One data source failure must never cascade to unrelated systems
- Always wrap per-asset operations in try/except

### v4.0

**Two-Phase Prescreener**
- New `prescreener.py` scores all ~230 Aftermath assets in one API call (~6s).
...
- Tier 1 scanners → isolated sessions, `delivery.mode: "none"`, explicit the configured model
- Tier 2 decision-makers → isolated sessions, `delivery.mode: "announce"`, the configured model
