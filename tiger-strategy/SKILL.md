...
name: tiger-strategy
description: >-
  TIGER v2 — Multi-scanner trading system for Aftermath perpetuals via Senpi MCP.
...
**CRITICAL:** Do NOT use `estimatedFundingRate` from Aftermath ticker/market endpoints. It is 12.5x inflated versus actual funding settlement.

Use `/api/perpetuals/all-markets` and compute:

```ts
const ratePerPeriod = market.marketState.premiumTwap / market.indexPrice; // per 8h
const annualizedPct = Math.abs(ratePerPeriod) * 3 * 365 * 100;
```

- Funding arb scanner minimum (`30%` annualized): `abs(premiumTwap / indexPrice) >= 0.000274` per 8h period
- High daily yield (`>5%` on 10x margin): `abs(premiumTwap / indexPrice) >= 0.001667` per 8h period
- Risk Guardian funding-flip exit must check `premiumTwap` sign flip (or `ratePerPeriod` sign flip), not `estimatedFundingRate`

## Aftermath Margin and Leverage Rules

Aftermath is isolated-margin only. Every position has its own collateral allocation.

- For each new entry, pass explicit `collateralChange` in `create_position`.
- Unallocated collateral does not protect any open position.
- In multi-position mode (`maxSlots` > 1), each slot is an independent isolated-margin position.
- Before opening, call `POST /api/perpetuals/account/max-order-size` to confirm enough unallocated collateral.

Dynamic leverage rule (required on every entry):

```text
targetLev = scanner_or_config_target
maxLev = market.maxLeverage()
lev = min(targetLev, maxLev)
```

Never assume a global leverage cap. Limits are market-specific and can change.
...
**DSL note:** Wider retrace tiers (0.02+) — edge is income, not price direction. Risk Guardian auto-exits when `premiumTwap` sign flips.
...
| Funding reversal exit | `premiumTwap` sign flips on FUNDING_ARB | — | Auto |
...
3. **NEVER hold FUNDING_ARB after `premiumTwap` sign flips.** The thesis is dead.
...
| Max leverage target (runtime-clamped to market max) | `maxLeverage` | 7 | 10 | 10 |
