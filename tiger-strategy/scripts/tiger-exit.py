...
def evaluate_position(pos_data: dict, active_pos: dict, config: dict, state: dict) -> dict:
    ...
    # 1. Daily target hit → take profit (ONLY for T0/T1 positions)
    # v4: T2+ positions have proven momentum — let DSL manage them, don't cap winners
    current_tier_idx = tracked.get("current_tier_idx", -1)
    if unrealized_pnl > 0 and daily_target_usd > 0 and unrealized_pnl >= daily_target_usd and current_tier_idx < 2:
        actions.append({
            "type": "DAILY_TARGET_HIT",
            "action": "CLOSE",
            "reason": f"Position +${unrealized_pnl:.2f} hit daily target ${daily_target_usd:.2f}. Take profit. (T{current_tier_idx}, below T2 threshold)",
            "priority": "HIGH"
        })
    ...
    if high_water_roe > 5 and roe_pct > 0 and dsl_tiers:
        ...
        if active_tier:
            ...
            # Set exchange-level SL at DSL lock price (enforced in real-time by Aftermath Finance)
    ...
    # v4: Only for T0/T1 positions. T2+ positions have proven momentum — ATOM held at +10% for hours then ran to +16%.
    if roe_pct > 3 and abs(high_water_roe - roe_pct) < 1 and tracked.get("stagnant_checks", 0) >= 24 and current_tier_idx < 2:
        actions.append({
            "type": "STAGNATION",
            "action": "CLOSE",
            "reason": f"ROE stagnant at ~{roe_pct:.1f}% for 2+ hours (T{current_tier_idx}). Take profit and rotate.",
            "priority": "MEDIUM"
        })
    ...
    # 5a. NEVER-GREEN FAST CUT: 15min if trade never went positive
    # v4: Data shows ≤1h trades are 20% WR. If it never goes green, the signal was wrong.
    if opened_at and high_water_roe <= 0.5:
        try:
            opened = datetime.fromisoformat(opened_at)
            elapsed_min = (datetime.now(timezone.utc) - opened).total_seconds() / 60
            if elapsed_min >= 15:
                actions.append({
                    "type": "NEVER_GREEN_CUT",
                    "action": "CLOSE",
                    "reason": f"Never went positive after {elapsed_min:.0f}min (peak ROE {high_water_roe:.1f}%). Signal failed.",
                    "priority": "HIGH"
                })
        except:
            pass

    # 5b. Losing position past time limit (30 min + negative = cut)
...
def main():
    ...
    if isinstance(active_pos, list):
        active_pos = {p.get("coin", p.get("asset", "")): p for p in active_pos}
    exit_signals = []
