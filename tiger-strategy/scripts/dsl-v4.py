...
min_hold_minutes = state.get("minHoldMinutes", 0)
...
for i, tier in enumerate(tiers):
    if i <= tier_idx:
        continue
    if upnl_pct >= tier["triggerPct"] * 100:
        tier_idx = i
        tier_changed = True
        if is_long:
            tier_floor = round(entry * (1 + tier["lockPct"] / state["leverage"]), 4)
        else:
            tier_floor = round(entry * (1 - tier["lockPct"] / state["leverage"]), 4)
...
if phase == 1:
    ...
    if tier_idx >= 0:
        # v4: Per-tier retrace from config (low tiers breathe, high tiers lock tight)
...
# ─── Minimum hold time ───
hold_elapsed_minutes = 0
if state.get("createdAt"):
    try:
        created = datetime.fromisoformat(state["createdAt"].replace("Z", "+00:00"))
        hold_elapsed_minutes = (datetime.now(timezone.utc) - created).total_seconds() / 60
    except (ValueError, TypeError):
        pass

in_hold_period = min_hold_minutes > 0 and hold_elapsed_minutes < min_hold_minutes
...
# During hold period, only close on absolute floor breach (hard SL), not trailing
if in_hold_period and breached and phase == 1:
    abs_floor = state["phase1"]["absoluteFloor"]
    if is_long:
        hard_breached = price <= abs_floor
    else:
        hard_breached = price >= abs_floor
    if not hard_breached:
        breached = False  # Suppress trailing breach during hold period
...
tier_name = f"Tier {tier_idx+1} ({tiers[tier_idx]['triggerPct']*100:.0f}%→lock {tiers[tier_idx]['lockPct']*100:.0f}%)" if tier_idx >= 0 else "None"

previous_tier_name = None
if tier_changed:
    if previous_tier_idx >= 0:
        t = tiers[previous_tier_idx]
        previous_tier_name = f"Tier {previous_tier_idx+1} ({t['triggerPct']*100:.0f}%→lock {t['lockPct']*100:.0f}%)"
    else:
...
if next_tier_idx < len(tiers):
    distance_to_next_tier = round(tiers[next_tier_idx]["triggerPct"] * 100 - upnl_pct, 2)
