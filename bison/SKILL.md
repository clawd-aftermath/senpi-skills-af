...
name: bison-strategy
description: >-
  BISON v1.1 — Conviction holder for top 10 Aftermath assets by volume. Enters on 4h trend structure
...
Minimum score: 8. Conviction-scaled margin allocation (isolated per position): 25% base, 31% at score 10, 37% at score 12+.

For each entry, pass the selected allocation explicitly as `collateralChange` and confirm unallocated capacity with `POST /api/perpetuals/account/max-order-size` before opening.
...
- For funding direction checks, use `premiumTwap` sign from `/api/perpetuals/all-markets` (`premiumTwap / indexPrice`), not `estimatedFundingRate`
- "Funding extreme against" thesis exits must trigger on `premiumTwap` sign reversal against the position direction
