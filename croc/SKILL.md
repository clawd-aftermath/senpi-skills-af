...
**CRITICAL:** Do NOT use `estimatedFundingRate` from ticker/market endpoints. On Aftermath it is 12.5x inflated versus real settlement.
...
- Entry threshold mapping: `20%` annualized means `abs(premiumTwap / indexPrice) >= 0.000183` per 8h period
- Funding direction should come from the sign of `premiumTwap` (or `ratePerPeriod`), not from `estimatedFundingRate`
