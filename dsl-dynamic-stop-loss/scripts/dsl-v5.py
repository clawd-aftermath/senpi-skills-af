...
def sync_sl_to_aftermath(
    ...
    """Set or update SL on Aftermath Finance at effective_floor via edit_position. Resolve slOrderId via open orders if needed.
    ...
    # AFTERMATH OPTIMIZATION: On Aftermath Finance, the initial SL is placed inline
    # with the entry order via the `slTp` field on place-limit-order / place-market-order.
    # This function handles TRAILING updates only — the entry-time SL is already set.
    # For trailing, we use edit-stop-orders via MCP edit_position.
    #
    # Aftermath order type mapping for SL:
    #   Phase 1 (protecting capital): orderType "MARKET" -> Aftermath IOC (3)
    #   Phase 2 (trailing profit):    orderType "LIMIT"  -> Aftermath PostOnly (2) for better fill
    ...
    if oid is None:
        ...
        # Use rounded price to match what was sent to Aftermath Finance (round(..., 4))
...
def process_one_position(state_file: str, strategy_id: str, now: str) -> None:
    ...
    # Sync SL to Aftermath Finance: never synced or floor changed (compare rounded to match stored lastSyncedFloorPrice)
    need_sync = (
    ...
    if need_sync:
        sync_ok, sl_synced_this_tick, sync_err = sync_sl_to_aftermath(
