...
if abs(new_floor_price - state.get("lastSyncedFloorPrice", 0)) > 0.01:
    sync_sl_to_aftermath(wallet, coin, new_floor_price)
...
- [x]  High water advances → SL synced to Aftermath at new floor — `test_checklist_5_high_water_advances_sl_would_sync` (asserts need_sync when floor changes)
