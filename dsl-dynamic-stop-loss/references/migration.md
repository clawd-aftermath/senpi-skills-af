# Upgrading / Migration (Aftermath SL flow)

See the main skill: [SKILL.md](../SKILL.md).

This document describes how to move from an earlier DSL version (cron-only breach detection and `close_position` on breach) to the current flow where the stop loss is synced to Aftermath via Senpi `edit_position` and Aftermath executes the SL when price hits.

## Summary

- **No separate migration script or manual state edits.** The main script ([scripts/dsl-v5.py](../scripts/dsl-v5.py)) performs migration on the first run after the update by syncing each position’s SL to Aftermath and backfilling state.
...
2. It calls `edit_position` to place the current effective floor as a stop loss on Aftermath.
3. It writes `slOrderId`, `lastSyncedFloorPrice`, and `slOrderIdUpdatedAt` into the state file.

From that run onward, the position is protected by Aftermath’s native SL and the script only updates the SL when the floor changes. The script output may include `sl_initial_sync: true` for positions that were migrated this run (see [output-schema.md](output-schema.md)).

## Migrate immediately (optional)

To sync SL to Aftermath without waiting for the next cron tick, run the same command the cron uses **once per strategy**, e.g.:
...
- The next cron run (or a manual run of `dsl-v5.py`) will use the new tiers and sync the SL to Aftermath based on the High Water floor.
...
- **Agent visibility:** Use `sl_initial_sync: true` in the output to optionally notify the user that a position was just moved to the Aftermath SL flow (see [output-schema.md](output-schema.md) Agent Response Logic).
