...
| `sl_initial_sync` | bool | True if this tick was the first sync for this position (e.g. after upgrading to the Aftermath SL flow); state now has `slOrderId` and the position is protected by AF native SL |
