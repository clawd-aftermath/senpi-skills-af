...
Two-phase trailing stop for Aftermath perps. Protects profits, limits losses, and syncs the stop loss directly to the exchange so positions are protected even if the cron process goes down.
...
**Exchange SL sync.** DSL doesn't just track the floor internally — it sets the actual stop loss on Aftermath via `edit_position`. If the cron process crashes, AF still executes the SL at the last synced price. Phase 1 uses MARKET orders (fast exit on loss). Phase 2 uses LIMIT orders (fee-optimized exit on profit).
...
After setting a stop loss via `edit_position`, DSL now calls `execution_get_order_status` to verify the SL was accepted. If the SL was filled (AF executed it between ticks), the state file is archived as `{asset}_archived_sl_{epoch}.json` — distinguishing exchange-executed stops from script-executed stops.
...
| `_archived_sl_{epoch}` | Aftermath executed the SL order |
...
| File | Purpose |
|---|---|
| `scripts/dsl-v5.py` | Cron runner — monitors positions, syncs SL to AF, closes on breach |
...
- Aftermath perp positions (main dex + xyz dex)
