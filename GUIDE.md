...
## Aftermath Isolated Margin Model

Aftermath perpetuals use isolated margin only. There is no cross-margin mode.

Collateral flow:

```text
Wallet USDC -> Deposit -> Account (unallocated collateral)
...
- Unallocated collateral does not protect any open position until explicitly allocated.
- Every entry should pass explicit `collateralChange` for that position.
- A liquidation in one market does not consume collateral from other isolated positions.

Margin health zones (vs maintenance margin ratio):

- Safe: margin ratio > 2.0x maintenance
- Warning: 1.5x-2.0x maintenance
- Danger: 1.0x-1.5x maintenance
- Liquidation: < 1.0x maintenance

Max-order-size check pattern (before entry):

```text
1) Query POST /api/perpetuals/account/max-order-size
2) Confirm requested size at target leverage fits unallocated collateral
3) If insufficient, reduce size/leverage or skip
```

Always treat leverage as market-specific: fetch `maxLeverage` from market params and clamp requested leverage at runtime.
...
## Aftermath WebSocket for Real-Time Data
...
NOTE: asks can arrive in descending order. ALWAYS sort asks ascending before using best ask.
...
## Aftermath Order Types & Fee Optimization

When building skills against Aftermath native perps endpoints, use the correct numeric order type mapping:

| Value | Type | Usage |
|---|---|---|
| `0` | GTC (Good Till Cancelled) | Standard resting limit order |
| `1` | FOK (Fill or Kill) | Must fully fill immediately or fail |
| `2` | PostOnly (maker-only) | Reject if crossing; maker-fee optimization path |
| `3` | IOC (Immediate or Cancel) | Fill immediately (partial allowed), cancel remainder |
...
Use scale-order endpoint when placing multi-level entries:

- Endpoint: `POST /api/perpetuals/account/transactions/place-scale-order`
- Places the full ladder in a single transaction
- `sizeSkew` controls allocation across levels (`1.0` uniform, `2.0` means last level is 2x first)

### Fee + Gas Breakeven Check

Before adding extra maker-management transactions, estimate:

```text
minProfitableSize = gasCost / (takerFee - makerFee)
```

If intended notional is below this threshold, immediate execution (`IOC` / `MARKET`) may be cheaper overall than strict maker optimization.
