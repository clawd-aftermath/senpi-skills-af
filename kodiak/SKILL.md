...
## Why SOL-Only with Runtime-Verified Leverage

- **High volatility** — SOL moves 5-10% in hours, amplifying conviction trades
- **Growing on-chain liquidity on Aftermath** — increasingly tradeable at size
...
- **Lower leverage compensates for volatility** — use target leverage, then clamp to market max at runtime
...
Use isolated collateral allocation per position (`collateralChange`) and verify capacity with `POST /api/perpetuals/account/max-order-size` before entry.
