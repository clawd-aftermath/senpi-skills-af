...
name: wolf-strategy
description: >-
  WOLF v6.3 — Fully autonomous multi-strategy trading for Aftermath perpetuals via Senpi MCP.
  ...
  (native Aftermath SL sync via dsl-dynamic-stop-loss skill v5.3.1). Same asset can be traded in
...
**v6.2: DSL v5.2 integration.** Replaced internal `dsl-combined.py` with the `dsl-dynamic-stop-loss` skill. Native Aftermath stop-loss sync via `dsl-v5.py`.
...
| 2 | DSL *(per strategy)* | **3min** | isolated | `dsl-v5.py` (DSL skill) | Trailing stops + native Aftermath SL sync for ONE strategy's positions |
...
- **Native Aftermath SL:** dsl-v5.py sets a real stop-loss order on Aftermath via `edit_position`. Between cron runs, Aftermath's engine protects the position.
...
2. Enter via `python3 scripts/open-position.py --strategy {strategyKey} --asset {ASSET} --direction {DIR} --conviction {CONVICTION}`
   - Previews order first, then opens position with inline `slTp` (initial SL at Phase 1 floor), and calls `dsl-cli.py add-dsl` to create DSL state
   - DSL CLI fetches fill data (entryPx, size, leverage) directly from clearinghouse
   - Use isolated margin allocation per position by passing explicit `collateralChange` on entry
   - Before opening, check `POST /api/perpetuals/account/max-order-size` to confirm sufficient unallocated collateral
...
- Always set `leverageType: "ISOLATED"` and pass explicit `collateralChange` for each position
- Unallocated collateral is not protective; verify available size via `POST /api/perpetuals/account/max-order-size`
...
`maxLeverage` must come from Aftermath market params at runtime (`market.maxLeverage()` / `market.marketParams.marginRatioInitial`).
...
- **Watchdog blind spot for isolated liquidation distances:** The watchdog can't infer per-position liquidation distance equally well across all markets. Positions rely on DSL for protection.
...
| *(DSL skill)* `dsl-v5.py` | DSL cron runner: trailing stops + native Aftermath SL sync, one per strategy |
