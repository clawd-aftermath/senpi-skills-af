...
def sync_sl_to_aftermath(state, effective_floor, now, dex, phase):
...
def sync_native_tp(state, now):
    """Set a native take-profit on Aftermath Finance via edit_position.
...
def process_one_position(state_file, strategy_id, now):
    ...
    if need_sync and position_age_min >= min_age_for_sl_sync:
        ok, sl_synced, sync_err = sync_sl_to_aftermath(state, effective_floor, now, dex, phase)
