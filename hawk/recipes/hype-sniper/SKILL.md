...
**Aftermath-native SL + TP:**
- Stop loss synced to Aftermath via `edit_position(stopLoss)`
- Take profit set at next tier target via `edit_position(takeProfit)` — Aftermath executes natively, no cron dependency
