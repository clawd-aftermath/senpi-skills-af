...
class ProvenanceTests(unittest.TestCase):
    ...
    def test_historical_api_provenance_is_exact(self) -> None:
        api = json.loads(
            (ROOT / "aftermath-overlay/provenance.json").read_text()
        )["aftermath_api"]
        historical_site = "https://v2-" + "preview.aftermath.finance"
        self.assertEqual(historical_site, api["site"])
        self.assertEqual(
            historical_site + "/api/openapi/spec.json",
            api["openapi_url"],
        )
