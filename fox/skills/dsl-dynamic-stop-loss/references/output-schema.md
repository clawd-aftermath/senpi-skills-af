...
### v5 Aftermath SL Fields

| Field | Type | Description |
|-------|------|-------------|
| `sl_synced` | bool | True if the SL was synced to Aftermath this tick (via `edit_position`) |
| `sl_initial_sync` | bool | True if this tick was the first sync for this position (e.g. after upgrading to the Aftermath SL flow); state now has `slOrderId` and the position is protected by native Aftermath SL |
| `sl_order_id` | int/null | Aftermath order ID of the current SL for this position; null if not yet set or unavailable |
...
if sl_initial_sync == true:
  → optionally notify user "Trailing stop is now synced to Aftermath for this position; it will close at the stop level even between cron runs."
